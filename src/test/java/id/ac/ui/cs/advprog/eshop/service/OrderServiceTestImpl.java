...
class OrderServiceTest {
    ...
    void testUpdateStatus() {
        ...
        Order newOrder = new Order(order.getId(), order.getProducts(), order.getOrderTime(),
            order.getAutor(), OrderStatus.SUCCESS.getValue());
        doReturn(order).when(orderRepository).findById(order.getId());
    ...
    void testFindAllByAuthorIfAuthorCorrect() {
        Order order = orders.get(1);
        doReturn(orders).when(orderRepository).findAllByAuthor(order.getAutor());

        List<Order> results = orderService.findAllByAuthor(order.getAutor());
        for (Order result : results) {
            assertEquals(order.getAutor(), result.getAutor());
        }
    ...
    void testFindAllByAuthorIfAllLowercase() {
        ...
        doReturn(new ArrayList<Order>()).when(orderRepository)
            .findAllByAuthor(order.getAutor().toLowerCase());

        List<Order> results = orderService.findAllByAuthor(
            order.getAutor().toLowerCase());
